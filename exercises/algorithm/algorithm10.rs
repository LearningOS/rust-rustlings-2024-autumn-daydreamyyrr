...
impl Graph for UndirectedGraph {
    ...
    fn add_edge(&mut self, edge: (&str, &str, i32)) {
        ...
        self.add_node(node1);  // 确保节点 node1 存在
        self.add_node(node2);  // 确保节点 node2 存在

        let node1 = String::from(node1);
        let node2 = String::from(node2);

        // 添加从 node1 到 node2 的边
        self.adjacency_table.get_mut(&node1).unwrap().push((node2.clone(), weight));

        // 添加从 node2 到 node1 的边
        self.adjacency_table.get_mut(&node2).unwrap().push((node1, weight));
    }
}
